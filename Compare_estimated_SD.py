...
class SimulPivotMC(object):
    ...
    def main(self, method):
        ...
        if method == 'no_moments': 
            ...
            df[['rSampleMeanRaw', 'rSampleSDRaw']] = df['rSampleOfRandoms'].apply(self.Mean_SD).tolist()

            return df[['rSampleMeanRaw', 'rSampleSDRaw']], self.nMonte, self.N1, self.CV1, method
        # using method of moments to transform from raw to log mean and SD
            ...
            if method in ['Luo_Wan', 'bc', 'qe', 'mln']:
                ...
                # calculate sample mean and SD using Mean_SD
                df[['rSampleMeanRaw', 'rSampleSDRaw']] = df['rSampleOfRandoms'].apply(self.get_estimated_Mean_SD_from_Samples, args=(method,)).tolist()
            ...
            return df_record, self.nMonte, self.N1, self.CV1, method
    ...
    for method in ['no_moments']:
        ...
        for N in [51]: 
            ...
            for CV in [0.5]: 
                ...
                # Cal the class SimulPivotMC(), generate variables in th def __init__(self)
                ...
                dict_data[f'{method}_{N1}_{CV1}_SD_percentile'] = [df_record['rSampleSDRaw'].percentile(p) for p in range(1,100)]

    df = pd.DataFrame(dict_data)

    output_dir = f"Compare_estimated_SD_{nMonte}_N_{N1}_CV_{CV1}_{str(end_time).split('.')[0].replace('-','').replace(' ','').replace(':','')}"
    folder = "Compare_estimated_SD_20231116"
    output_dir = os.path.join(folder, output_dir)

    # save the results to the csv
    df.to_csv(output_dir + f'_{method}.csv')
